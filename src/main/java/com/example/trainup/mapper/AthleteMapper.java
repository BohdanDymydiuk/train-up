...
public interface AthleteMapper {
        @Mapping(source = "userCredential.email", target = "email")
        ...
        default Set<Sport> mapSportIdsToSport(Set<Long> sportIds, @Context SportRepository sportRepository) {
                if (sportIds == null || sportIds.isEmpty()) {
                        return Set.of();
                }
                return sportIds.stream()
                        .map(id -> sportRepository.findById(id)
                                .orElseThrow(() -> new IllegalArgumentException("Sport with id: " + id + "not found")))
                        .collect(Collectors.toSet());
        }
